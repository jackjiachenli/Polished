    ...
    func handleDragStart(windowID: CGWindowID, initialRect: CGRect, window: AXUIElement, currentRect: CGRect, cursor: NSPoint) {
        if let lastSnap = lastSnapRects[windowID],
           ...
           sizeDiffers(restore.size, initialRect.size) {
            WindowAX.restoreSizeWhileDragging(
...
private enum WindowAX {
    ...
    static func windowAtCursor(_ point: NSPoint) -> AXUIElement? {
        ...
        for _ in 0..<24 {
            if isSnappable(current) { return current }
    ...
    static func isSnappable(_ window: AXUIElement) -> Bool {
        ...
        return isSettable(window, kAXPositionAttribute as CFString)
    ...
    /// Whether the frontmost snappable window is fullscreen (disables drag-snapping).
    ...
    static func frame(of window: AXUIElement) -> CGRect? {
        ...
        return CGRect(x: pos.x, y: ScreenCoordinates.primaryMaxY - pos.y - size.height, width: size.width, height: size.height)
    }
    ...
    /// Restore pre-snap dimensions while dragging, keeping the cursor on the title bar when possible.
...
final class WindowSnapper: Module {
    ...
    private func onMouseDragged(at point: NSPoint) {
        ...
        guard let window = capturedWindow, let currentRect = WindowAX.frame(of: window) else { return }
        ...
        if !windowMoving {
            if let initial = initialWindowRect,
               ...
               currentRect.origin != initial.origin {
                windowMoving = true
                if let windowID = capturedWindowID ?? WindowAX.windowID(of: window) {
    ...
    private func onMouseUp(at point: NSPoint) {
        ...
        guard !snappingDisabled,
              ...
              WindowAX.isSnappable(window) else { return }
        ...
        guard let currentRect = WindowAX.frame(of: window),
              let initial = initialWindowRect,
              currentRect.size == initial.size,
              currentRect.origin != initial.origin else { return }

        if !windowMoving, let windowID = capturedWindowID ?? WindowAX.windowID(of: window) {
            snapHistory.handleDragStart(
                windowID: windowID,
                initialRect: initial,
                window: window,
    ...
    private func applySnap(_ snapArea: SnapArea, to window: AXUIElement) {
        let windowID = capturedWindowID ?? WindowAX.windowID(of: window)
        if let windowID {
            capturedWindowID = windowID
        ...
        let target = SnapFrameCalculation.frame(for: snapArea.action, on: snapArea.screen)
        WindowAX.setFrame(target, on: window)

        if let windowID {
            let snapped = WindowAX.frame(of: window) ?? target
